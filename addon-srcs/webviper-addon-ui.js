...
// Globals
// -----------------------------------------------------------------------------------------
const useBrowserBackend = true; // Set to "false" for UI browser debugging
...
async function loadSettings() {
  ...
  // ---------------------------------------------------------------------------------
  controlUrlRuleList.appendChild(
    createSelectOption("", "No selection", true, false),
  );
...
async function saveSetting(settingName, value) {
  ...
  if (!hasError) {
    console.debug(
      `[webViper] [ SAVE ] [ SUCCESS ] Saved setting  '${settingName}' with value ==> ${value}`,
    );
    return true;
  }

  console.debug(
    `[webViper] [ SAVE ] [ ERROR ] Could not save setting  '${settingName}' with value ==> ${value}`,
  );
  return false;
}
...
const navElementContainers = document.querySelector(
  '#navigation a[href="#elementContainers"]',
);
const navReplaceElements = document.querySelector(
  '#navigation a[href="#replaceElements"]',
);
const navGlobalKeywords = document.querySelector(
  '#navigation a[href="#globalKeywords"]',
);
...
controlSaveAll.addEventListener("click", function (evt) {
  ...
  for (const option of elementContainersOptions) {
    ...
    if (isAdded) {
      elementContainersRules[modifiedKey] = modifiedValue
        .trim()
        .split(cssArrayDelimeter);
    } else {
      elementContainersRules[modifiedKey] = modifiedValue
        .trim()
        .split(cssArrayDelimeter);
  ...
  newCreatedRuleDictionary[targetURL].elementContainers =
    elementContainersRules;
...
function setActiveLinkAndScroll(targetId) {
  ...
  const activeNavItem = document.querySelector(
    `#navigation a[href="#${targetId.replace("-section", "")}"]`,
  );
...
function initUI() {
  ...
  const controls = document.querySelectorAll(
    ".actionItem input, .actionItem textarea, .actionItem select, .actionItem button",
  );
        ...
        if (parent.id !== currentActiveSectionId) {
          ...
          if (!isNavItemDisabled(navTarget)) {
            ...
            if (directionIsForward) {
              ...
              while (nextNavNode) {
                if (
                  nextNavNode.nodeName === "LI" &&
                  !hasClass(nextNavNode, "disabled")
                ) {
  ...
  // ---------------------------------------------------------------------------------------
  controlRuleSelector.addEventListener(
    "focus",
    function (evt) {
      evt.preventDefault();
    },
  ...
  controlElementContainers.addEventListener("change", function (evt) {
    if (evt.target.options.selectedIndex !== 0) {
      ...
      controlSelectorParents.value = getDatasetValue(option, modifiedValueStore)
        .split(cssArrayDelimeter)
        .join("\n");
      setDisableState(controlRemoveSelectorRule, false);
  ...
  controlSelectorParents.addEventListener("keyup", function (evt) {
    if (controlElementContainers.options.selectedIndex !== 0) {
      const option =
        controlElementContainers.options[
          controlElementContainers.options.selectedIndex
        ];

      const originalValue = getDatasetValue(option, originalValueStore);
      const compareValue = evt.target.value
        .trim()
        .split("\n")
  ...
  controlAddSelector.addEventListener("click", function (evt) {
    if (controlAddSelector.dataset.restore === "false") {
      ...
      if (!hasOption) {
        const selectorValues = controlSelectorParents.value
          .trim()
          .split("\n")
          .join(cssArrayDelimeter);
        const selectorOption = createSelectOption(
          selectorValues,
          selectorKey,
          false,
          false,
      ...
      if (controlAddSelector.dataset.restore === "true") {
        ...
        if (!isOriginal) {
          ...
          if (getDatasetValue(option, "added") === "true") {
            ...
            controlSelectorParents.value = compareValue
              .split(cssArrayDelimeter)
              .join("\n");
          ...
          setDataValue(option, modifiedValueStore, originalValue);
          controlSelectorParents.value = originalValue
            .split(cssArrayDelimeter)
            .join("\n");
        }
    ...
    if (controlElementContainers.options.selectedIndex !== 0) {
      ...
      if (getDatasetValue(option, "added") === "true") {
        ...
        if (!isRemoved) {
          ...
          controlSelectorParents.value = modifiedValue
            .split(cssArrayDelimeter)
            .join("\n");
        }
      }
  ...
  */
  ...
  controlClearSelector.addEventListener("click", function (evt) {
    if (controlElementContainers.options.selectedIndex !== 0) {
      ...
      controlRuleSelector.value = "";
      setDisableState(controlClearSelector, true);
      controlElementContainers.value = "";
      controlElementContainers.dispatchEvent(new Event("change"));
    }
  ...
  controlGlobalKeywords.addEventListener("keyup", function (evt) {
    setDisableState(
      controlSaveGlobalKeywords,
      isEqual(
        evt.target.value,
        getDatasetValue(evt.target, originalValueStore),
      ),
    );
  });
...
function setExpanderValue(target, value) {
  const expander = target.parentNode.classList.contains("expand-input")
    ? target.parentNode
    : false;
...
function searchRules(evt) {
  ...
  if (searchValue.length < 2) {
    ...
    controlNewURLRuleAction.value = getDatasetValue(
      controlNewURLRuleAction,
      originalValueStore,
    );
...
function deleteRule(evt) {
  ...
  if (
    window.confirm("Do you want to remove the rule for '" + ruleValue + "'")
  ) {
    clearRuleValues();
    clearSearchRule();
...
function hasFormDataChanged() {
  const dataHolders = document.querySelectorAll(
    ".actionItem input, .actionItem textarea, .actionItem select",
  );
  ...
  for (const item of dataHolders) {
    ...
    if (item.nodeName === "SELECT") {
      if (item.options.length !== 0) {
        ...
        if (item.id === "elementContainers") {
          for (const option of item.options) {
            ...
            if (
              ...
              isRemoved ||
              !isEqual(
                getDatasetValue(option, originalValueStore),
                getDatasetValue(option, modifiedValueStore),
              )
            ) {
              isDirty = true;
        ...
        if (
          hasDirtyOptions ||
          !isEqual(compareValue, getDatasetValue(compareTarget, compareStore))
        ) {
          isDirty = true;
        }
...
function clearRuleValues() {
  ...
  controlSelectorParents.value = "";
  setDataValue(
    controlReplaceElements,
    originalValueStore,
    controlReplaceElements.value,
  );

  controlElementContainers.replaceChildren();
  controlElementContainers.appendChild(
    createSelectOption("", "Select selector rule for edit", true, true),
  );
  setDataValue(
    controlElementContainers,
    originalValueStore,
    controlElementContainers.length,
  );
...
function setRuleValues(ruleKey) {
  if (!Object.hasOwn(ruleSet, ruleKey)) {
    console.debug(
      "[webViper] [ RULES ] Rule for set values is not present: " + ruleKey,
    );
    return;
  }
  ...
  if (
    Object.hasOwn(rule, "elementContainers") &&
    Object.keys(rule.elementContainers).length !== 0
  ) {
    Object.keys(rule.elementContainers).forEach((selectorKey) => {
      const selectorValues =
        rule.elementContainers[selectorKey].join(cssArrayDelimeter);
      const selectorOption = createSelectOption(
        selectorValues,
        selectorKey,
        false,
